...
public class MainCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        int LEXP = uc.LevelConfig.getInt("LevelingEXP");
        if (args.length == 0) {
            if(sender.hasPermission("lp.lps")) {
                for (String x : uc.LangConfig.getStringList("lp")) {
        ...
        if(args.length >= 1) {
            if (sender.hasPermission("lp.admin.reload")) {
                if (args.length == 1) {
                    if (args[0].equalsIgnoreCase("reload")) {
                        ...
                        uc.RunFiles();
                        sender.sendMessage(API.format(uc.LangConfig.getString("lpreload")));
                    }
            ...
            if (sender.hasPermission("lp.admin.give")) {
                ...
                if (args[0].equalsIgnoreCase("expgive")) {
                    if (args.length == 1) {
                        sender.sendMessage(API.format(uc.LangConfig.getString("lpsEXPGIVEPlayer")));
                        return true;
                    }

                    if (args.length == 2) {
                        sender.sendMessage(API.format(uc.LangConfig.getString("lpsEXPGIVEAmount")));
                        return true;
                    }
                    if (args.length == 3) {
                        ...
                        if (target != null) {
                            ...
                            sender.sendMessage(API.format(uc.LangConfig.getString("lpAdminEXPGive").replace("{LP_TARGET}", target.getName()).replace("{EXP_AMOUNT}", args[2])));
                            target.sendMessage(API.format(uc.LangConfig.getString("lpEXPGive").replace("{EXP_Amount}", args[2]).replace("{LP_USER}", uc.LangConfig.getString("lpServerName"))));
            ...
            if (sender.hasPermission("lp.admin.remove")) {
                if (args[0].equalsIgnoreCase("expremove")) {
                    ...
                    if (args.length == 1) {
                        sender.sendMessage(API.format(uc.LangConfig.getString("lpsEXPREMOVEPlayer")));
                        return true;
                    }
                    if (args.length == 2) {
                        sender.sendMessage(API.format(uc.LangConfig.getString("lpsEXPREMOVEAmount")));
                        return true;
                    }
                    if (args.length == 3) {
                        Player target = Bukkit.getPlayer(args[1]);
                        ...
                        if (sender instanceof Player) {
                            target.sendMessage(API.format(uc.LangConfig.getString("lpEXPRemove").replace("{EXP_AMOUNT}", args[2]).replace("{LP_USER}", sender.getName())));
                            sender.sendMessage(API.format(uc.LangConfig.getString("lpAdminEXPRemove").replace("{LP_TARGET}", target.getName()).replace("{EXP_AMOUNT}", args[2])));
                        }else{
                            target.sendMessage(API.format(uc.LangConfig.getString("lpEXPRemove").replace("{EXP_AMOUNT}", args[2]).replace("{LP_USER}", uc.LangConfig.getString("lpServerName"))));
                            sender.sendMessage(API.format(uc.LangConfig.getString("lpAdminEXPRemove").replace("{LP_TARGET}", target.getName()).replace("{EXP_AMOUNT}", args[2])));
                        }
            ...
            if (sender.hasPermission("lp.player")) {
                if (args[0].equalsIgnoreCase("top")) {
                    ...
                    for (String x : uc.LangConfig.getStringList("lpsTopListTop")) {
                        sender.sendMessage(API.format(x));
                    ...
                    cf.getValues(false)
                            ...
                            .forEach(f -> {
                                ...
                                for (String x : uc.LangConfig.getStringList("lpsTopListMid")) {
                    ...
                    for (String x : uc.LangConfig.getStringList("lpsTopListBottom")) {
                        sender.sendMessage(API.format(x));
                        // Here you can send to player or do whatever you wan't.
                ...
                if (args.length >= 1) {
                    ...
                    if (args.length == 1) {
                        if (args[0].equalsIgnoreCase("prestige")) {
                            if (UsersConfig.getInt("Level") == uc.getMaxLevel()) {
                                if (UsersConfig.getInt("EXP.Amount") >= uc.getMaxLevelEXP(player)) {
                                    ...
                                    player.sendMessage(API.format(uc.LangConfig.getString("lpsPrestigeLevelUP").replace("{PRESTIGE_AMOUNT}", String.valueOf(1))));
                                    return true;
                                } else {
                                    int need = uc.getRequiredEXP(player) - uc.getCurrentEXP(player);


                                    player.sendMessage(API.format(uc.LangConfig.getString("lpsPrestigeMoreEXP").replace("{EXP_AMOUNT}", String.valueOf(need))));
                                }
                            } else {
                                player.sendMessage(API.format(uc.LangConfig.getString("lpsPrestigeLevelNot").replace("{MAX_LEVEL}", String.valueOf(uc.getMaxLevel()))));
                            }
                        }
                        if (args[0].equalsIgnoreCase("info")) {
                            ...
                            for (String x : uc.LangConfig.getStringList("lpsInfo")) {
                        ...
                        if (args[0].equalsIgnoreCase("toggle")) {
                            ...
                            player.sendMessage(API.format(uc.LangConfig.getString("lpsActionBarToggle").replace("{LP_Toggle_Value}", String.valueOf(UsersConfig.getBoolean(player.getName() + ".ActionBar")))));
                ...
                if (args.length == 2) {
                    if (args[0].equalsIgnoreCase("info")) {
                        ...
                        if (Bukkit.getPlayer(str.toString()) != null) {
                            ...
                            sender.sendMessage(API.format(uc.LangConfig.getString("LPSNotOnline")));
                        }
                    }
                }
                if(args.length == 5){
                        ...
                        if(args[0].equalsIgnoreCase("booster")){
                            if(args[1].equalsIgnoreCase("give")){
                                ...
                                if(Target != null) {
                                    ...
                                    Target.sendMessage(API.format(uc.LangConfig.getString("BoosterGive")).replace("{BoosterMultiplier}", String.valueOf(Multipler)));
                                }
                            }
                        }
                    }
                        ...
                        if (args[0].equalsIgnoreCase("booster")) {
                            ...
                            if (args[1].equalsIgnoreCase("list")) {
                                ...
                                for (String x : BoostersList.getKeys(false)) {
                                    ...
                                    player.sendMessage(API.format(uc.LangConfig.getString("lpsBoosterList").replace("{Booster_Multiplier}", x).replace("{Booster_Amount}", String.valueOf(amount))));
                                }
                            }
